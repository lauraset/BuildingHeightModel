...
from ptsemseg.models import TLCNetUtlcmux
...
def main():
    ...
    testdataloader = torch.utils.data.DataLoader(
        myImageFloder(testimg, testlab, num=16),
        batch_size=batch_size, shuffle=False, num_workers=4, pin_memory=True)

    # Setup Model
    model = TLCNetUtlcmux(n_classes=1).to(device)
    if torch.cuda.device_count() > 1:
        model = torch.nn.DataParallel(model, device_ids=range(torch.cuda.device_count()))

    # print the model
    start_epoch = 0
    resume = r'runs\tlcnetu_zy3bh_tlcmux\V1\finetune_298.tar'
